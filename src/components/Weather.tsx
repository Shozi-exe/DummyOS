...
const Weather = () => {
  ...
  return (
    <div className={`h-full w-full flex flex-col justify-evenly bg-linear-to-tr ${curr >= 22 || curr <= 6 ? "from-black via-blue-900 to-white" : "from-white via-blue-400 to-blue-500"} text-white`}>
      ...
      <section className="w-full h-50 flex justify-between items-center px-5">
        ...
        <aside className="relative flex items-center justify-center">
          <img
            className="h-38 w-38 object-contain drop-shadow-2xl transition-transform duration-300 hover:scale-105"
            src={currentWeather.iconUrl}
            alt={currentWeather.label}
          />
        </aside>
      </section>
      <section className="h-42 w-[95%] flex flex-col justify-evenly mx-auto px-7 bg-[#456f8928] rounded-xl mt-20">
        ...
        <main className="flex gap-3 overflow-scroll scrollbar-none">
          {cards.map((card, index) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col justify-evenly items-center m-3 w-20 h-25 shrink-0"
              >
